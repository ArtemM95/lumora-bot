        ...
        "zones": [
            {
                "title": "LinkedIn",
                "tasks": [
                    {"id": "li1", "name": "Обновить заголовок профиля на EN", "xp": 20, "freq": "once"},
                    {"id": "li2", "name": "Опубликовать 1 пост о своём опыте", "xp": 15, "freq": "daily"},
                    {"id": "li3", "name": "Отправить 10 личных DM в LinkedIn", "xp": 25, "freq": "daily"},
                ]
            },
            {
                "title": "Портфолио",
                "tasks": [
                    {"id": "pf1", "name": "Снять AI demo-ролик (2-3 мин)", "xp": 50, "freq": "once"},
                    {"id": "pf2", "name": "Добавить кейс на английском", "xp": 30, "freq": "once"},
                ]
            },
            {
                "title": "Upwork",
                "tasks": [
                    {"id": "up1", "name": "Заполнить профиль на Upwork", "xp": 25, "freq": "once"},
                    {"id": "up2", "name": "Подать заявки на 5 проектов", "xp": 20, "freq": "daily"},
                ]
            },
            {
                "title": "Английский",
                "tasks": [
                    {"id": "en1", "name": "30 мин английского (подкаст/Anki)", "xp": 10, "freq": "daily"},
                    {"id": "en2", "name": "Написать короткий текст на EN", "xp": 15, "freq": "daily"},
                ]
            },
        ]
    },
        ...
        "zones": [
            {
                "title": "Субподряд",
                "tasks": [
                    {"id": "ag1", "name": "Написать 5 агентствам напрямую", "xp": 30, "freq": "daily"},
                    {"id": "ag2", "name": "Написать партнёрский pitch 1 стр", "xp": 40, "freq": "once"},
                ]
            },
            {
                "title": "Контракты",
                "tasks": [
                    {"id": "ct1", "name": "Закрыть первый платный проект", "xp": 100, "freq": "once"},
                    {"id": "ct2", "name": "Запросить отзыв у клиента", "xp": 30, "freq": "once"},
                ]
            },
            {
                "title": "Контент",
                "tasks": [
                    {"id": "yt1", "name": "Снять BTS-видео процесса (60 сек)", "xp": 20, "freq": "daily"},
                    {"id": "yt2", "name": "Прокомментировать 5 постов клиентов", "xp": 10, "freq": "daily"},
                ]
            },
        ]
    },
        ...
        "zones": [
            {
                "title": "Ретейнеры",
                "tasks": [
                    {"id": "rt1", "name": "Предложить клиенту monthly retainer", "xp": 50, "freq": "once"},
                    {"id": "rt2", "name": "Описать 3 пакета услуг с ценами", "xp": 35, "freq": "once"},
                ]
            },
            {
                "title": "Релокация",
                "tasks": [
                    {"id": "rl1", "name": "Изучить регистрацию ИП в Сербии", "xp": 20, "freq": "once"},
                    {"id": "rl2", "name": "Открыть счёт Wise или Payoneer", "xp": 30, "freq": "once"},
                    {"id": "rl3", "name": "Доход 1500+ USD стабильно 2 мес", "xp": 100, "freq": "once"},
                ]
            },
            {
                "title": "Аналитика",
                "tasks": [
                    {"id": "an1", "name": "Считать: сколько DM дали ответ", "xp": 15, "freq": "daily"},
                    {"id": "an2", "name": "Записать главный инсайт недели", "xp": 10, "freq": "daily"},
                ]
            },
        ]
    },
...
def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            return json.load(f)
    return {"xp": 0, "streak": 0, "last_day": None, "total_days": 0, "done": {}, "once_done": {}, "chat_id": None, "phase": 0}
...
def build_today_keyboard(data):
    ...
    for task in tasks:
        ...
        keyboard.append([InlineKeyboardButton(f"{check} {task['name']}  +{task['xp']} XP", callback_data=f"task_{task['id']}")])
...
def build_stats_text(data):
    ...
    today = today_str()
    daily_tasks = get_daily_tasks(data)
    today_done = sum(1 for t in daily_tasks if t["done"])
    today_total = len(daily_tasks)
    ...
    return (
        ...
        f"Серия дней:       {streak} дн.\n"
        f"Всего дней:       {total_days} дн.\n"
        f"Разовых задач:    {done_once} выполнено\n\n"
        f"Сегодня: {today_done} из {today_total} задач\n"
        f"{'='*25}\n"
        f"Фаза: {phase_name}"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Привет, Артём!\n\n"
        f"Сегодня: {today}\n"
        f"Уровень {level + 1}: {LEVELS[level][1]}  |  {xp} XP\n"
        f"Серия: {data['streak']} дн.  |  Сегодня: {done_count}/{len(tasks)}\n\n"
        f"Задачи на сегодня:",
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "back":
        ...
        await query.edit_message_text(
            f"Уровень {level + 1}: {LEVELS[level][1]}  |  {xp} XP\n"
            f"Сегодня: {done_count}/{len(tasks)} задач",
            reply_markup=build_today_keyboard(data)
        )
        return
    ...
    if query.data.startswith("task_"):
        ...
        if was_done:
            ...
            if task["freq"] == "once" and task_id in data["once_done"]:
        ...
        xp = data["xp"]
        level = get_level(xp)
        msg = f"{'Готово! +' if not was_done else 'Отменено -'}{task['xp']} XP\n\n"
        msg += f"Уровень {level + 1}: {LEVELS[level][1]}  |  {xp} XP\n"
        msg += f"Сегодня: {done_count}/{len(tasks)} задач"
        ...
        "zones": [
            {
                "title": "LinkedIn",
                "tasks": [
                    {"id": "li1", "name": "Написать/обновить заголовок профиля", "xp": 20, "freq": "once"},
                    {"id": "li2", "name": "Опубликовать 1 пост о своём опыте", "xp": 15, "freq": "daily"},
                    {"id": "li3", "name": "Отправить 10 персональных DM", "xp": 25, "freq": "daily"},
                ]
            },
            {
                "title": "Портфолио",
                "tasks": [
                    {"id": "pf1", "name": "Сделать AI demo-ролик (2-3 мин)", "xp": 50, "freq": "once"},
                    {"id": "pf2", "name": "Добавить кейс на английском", "xp": 30, "freq": "once"},
                ]
            },
            {
            ...
            },
            {
                "title": "Английский",
                "tasks": [
                    {"id": "en1", "name": "30 минут английского (подкаст/Anki)", "xp": 10, "freq": "daily"},
                    {"id": "en2", "name": "Написать 1 текст на английском", "xp": 15, "freq": "daily"},
                ]
            },
        ]
    },
    {
        "name": "Месяц 2 — первые контракты",
        "zones": [
            {
                "title": "Субподряд",
                "tasks": [
            ...
            },
            {
                "title": "Контент",
                "tasks": [
                    {"id": "yt1", "name": "Снять BTS-видео (60-90 сек)", "xp": 20, "freq": "daily"},
                    {"id": "yt2", "name": "Прокомментировать 5 постов клиентов", "xp": 10, "freq": "daily"},
                ]
            },
        ]
    },
    {
        ...
        "zones": [
            ...
            },
            {
                ...
                "tasks": [
                    {"id": "rl1", "name": "Изучить регистрацию ИП в Сербии", "xp": 20, "freq": "once"},
                    {"id": "rl2", "name": "Открыть Wise/Payoneer", "xp": 30, "freq": "once"},
                    {"id": "rl3", "name": "Подтвердить доход 1500+ USD x 2 мес", "xp": 100, "freq": "once"},
                ]
            },
            {
                "title": "Аналитика",
                "tasks": [
                    {"id": "an1", "name": "Посчитать conversion rate DM → ответ", "xp": 15, "freq": "daily"},
                    {"id": "an2", "name": "Записать 1 инсайт недели", "xp": 10, "freq": "daily"},
                ]
            },
        ]
    },
...
def build_today_keyboard(data):
    ...
    for task in tasks:
        check = "✅" if task["done"] else "⬜"
        label = f"{check} {task['name']} (+{task['xp']} XP)"
        keyboard.append([InlineKeyboardButton(label, callback_data=f"task_{task['id']}")])
...
def build_stats_text(data):
    ...
    return (
        f"Твой прогресс, Артём\n\n"
        f"Уровень: {level + 1} — {level_name}\n"
        f"XP: {xp} (до следующего: {xp_to_next})\n"
        f"Серия дней: {streak}\n"
        f"Всего активных дней: {total_days}\n"
        f"Сегодня: {today_done}/{today_total} задач\n\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        f"Привет, Артём!\n\n"
        f"Сегодня {today}\n"
        f"Выполнено: {done_count}/{len(tasks)} задач\n\n"
        f"Отмечай задачи по мере выполнения:"
    )

    await update.message.reply_text(text, reply_markup=build_today_keyboard(data))
...
async def phase_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, phase in enumerate(PHASES):
        mark = "✅ " if i == data.get("phase", 0) else ""
        keyboard.append([InlineKeyboardButton(f"{mark}{phase['name']}", callback_data=f"phase_{i}")])
    await update.message.reply_text("Выбери текущую фазу плана:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "stats":
        await query.edit_message_text(
            build_stats_text(data),
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("← Назад к задачам", callback_data="back")
            ]])
        )
        return
    ...
    if query.data == "back":
        ...
        done_count = sum(1 for t in tasks if t["done"])
        await query.edit_message_text(
            f"Задачи на сегодня — выполнено {done_count}/{len(tasks)}:",
            reply_markup=build_today_keyboard(data)
        )
        return
    ...
    if query.data.startswith("phase_"):
        ...
        data["phase"] = phase_idx
        save_data(data)
        await query.edit_message_text(f"Фаза изменена: {PHASES[phase_idx]['name']}\n\nНапиши /today чтобы увидеть задачи.")
        return
    ...
    if query.data.startswith("task_"):
        ...
        xp_msg = f"+{task['xp']} XP!" if not was_done else f"-{task['xp']} XP"
        await query.edit_message_text(
            f"{'Готово!' if not was_done else 'Отменено'} {xp_msg}\n\nВыполнено: {done_count}/{len(tasks)} задач:",
            reply_markup=build_today_keyboard(data)
        )
...
async def send_morning_reminder(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"Доброе утро, Артём!\n\nСегодня {today}\nЗадач на день: {len(tasks)}\n\nОткрывай и отмечай:",
        reply_markup=build_today_keyboard(data)
    )
